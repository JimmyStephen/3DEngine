...
namespace nc
{
	void FreeCameraController::Update()
	{
		...
		//up down left right
		if (owner->scene->engine->Get<nc::InputSystem>()->GetKeyState(SDL_SCANCODE_W) == nc::InputSystem::eKeyState::Held) direction.y = 1;
		if (owner->scene->engine->Get<nc::InputSystem>()->GetKeyState(SDL_SCANCODE_S) == nc::InputSystem::eKeyState::Held) direction.y = -1;
		...
		//away/closer to camera
		if (owner->scene->engine->Get<nc::InputSystem>()->GetKeyState(SDL_SCANCODE_Q) == nc::InputSystem::eKeyState::Held) direction.z = 1;
		if (owner->scene->engine->Get<nc::InputSystem>()->GetKeyState(SDL_SCANCODE_E) == nc::InputSystem::eKeyState::Held) direction.z = -1;
